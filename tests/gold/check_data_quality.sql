...
/*
===================================================================
Test: gold.fact_listing row count matches silver.olx_apartments_info
===================================================================
Purpose:
...
DECLARE
    ...
    fact_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO silver_count FROM silver.olx_apartments_info;
    SELECT COUNT(*) INTO fact_count FROM gold.fact_listing;

    IF fact_count > silver_count THEN
        RAISE EXCEPTION
            'FAILED: gold.fact_listing has % rows but silver.olx_apartments_info has % — % rows were lost in the dimension lookup join',
